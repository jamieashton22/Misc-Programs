...
VID = 0x0483 #1155                  Vendor ID
PID = 0x5740 #22336                 Product ID 

# Get nanovna device automatically, automatically finds which port the nanoVNA is connected to by searching for a USB device w matching VID and PID
def getport() -> str:       # Function getport (returns string)
    device_list = list_ports.comports()     #list of connected serial devices 
    
    for device in device_list:
        if device.vid == VID and device.pid == PID:     # For each device check if VID and PID are correct 
            return device.device
    raise OSError("device not found")

REF_LEVEL = (1<<9)          # constant used to normalise/ scale 

class NanoVNA:          # define a new class called NanoVNA 
    def __init__(self, dev = None):             # Constructor, can optionally pass the name of the port the device is connected to, if not, getport handles it
        self.dev = dev or getport()             #self.dev holds device port path          
        self.serial = None                      
        self._frequencies = None                # initialises variable to store frequency points for a sweep
        self.points = 101                       # number of points in a frequency sweep is 101 by default 
        
    @property
    def frequencies(self):
        return self._frequencies        # clean way to access the frequencies being used 

    def set_frequencies(self, start = 1e6, stop = 900e6, points = None):        # method (member func) to set up frequency sweep range and number of points
        if points:
            self.points = points
        self._frequencies = np.linspace(start, stop, self.points)       # creates array _frequencies of equally spaced vals between start and stop 

    def open(self):                     # opens serial connection to NanoVNA device 
